...
const Services: React.FC = () => {
  return (
    <section id="servicos" className="py-24 bg-[#0B0B0B] border-y border-white/5 scroll-mt-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          ...
          <p className="text-slate-400 max-w-2xl mx-auto font-medium text-lg">
            Garantimos a conformidade legal e a eficiência financeira para que sua única preocupação seja vender mais.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {SERVICES.map((service) => (
            <div 
              key={service.id} 
              id={service.id} 
              className="bg-[#1A1A1A] p-10 rounded-[2.5rem] border border-white/5 hover:border-[#F7931E]/40 transition-all flex flex-col h-full scroll-mt-28 group hover:-translate-y-2 shadow-2xl"
            >
              <div className="bg-[#F7931E] text-black w-14 h-14 rounded-2xl flex items-center justify-center mb-8 shadow-[0_10px_20px_rgba(247,147,30,0.2)] group-hover:scale-110 transition-transform">
                {service.icon}
              </div>
        ...
        {/* Mensagem de Instrução Abaixo dos Blocos */}
